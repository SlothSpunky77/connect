...
void newWindow(int rows, int cols, int playersNos)
{
    ...
    gtk_widget_set_halign(gridTop, GTK_ALIGN_CENTER);
    gtk_widget_set_valign(gridTop, GTK_ALIGN_CENTER);
    ...
    gtk_widget_set_halign(gridBoard, GTK_ALIGN_CENTER);
    gtk_widget_set_valign(gridBoard, GTK_ALIGN_CENTER);
    gtk_grid_attach(GTK_GRID(main_grid), gridBoard, 0, 1, 1, 1);

    gridInput = gtk_grid_new();
    gtk_widget_set_halign(gridInput, GTK_ALIGN_CENTER);
    gtk_widget_set_valign(gridInput, GTK_ALIGN_CENTER);
...
int main(int argc, char *argv[])
{
    ...
    // Create a vertical box to center the contents vertically
    ...
    // Attach all widgets to the vertical box
    gtk_box_pack_start(GTK_BOX(vbox), important, FALSE, FALSE, 0);
    ...
    // Attach the vertical box to the main window
